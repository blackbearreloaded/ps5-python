...
class UnixSocket(BaseSocket):
    ...
    # AF_UNIX is not exposed by the PS5 socket build.  Keep the class
    # importable so TCP-only configurations can use the normal Gunicorn
    # module graph; _sock_type rejects Unix addresses below with a clear
    # platform error.
...
def _sock_type(addr):
    if isinstance(addr, tuple):
        ...
        if not hasattr(socket, "AF_UNIX"):
            raise RuntimeError("Unix-domain sockets are unavailable on PS5")

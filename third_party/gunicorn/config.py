...
_getegid = getattr(os, "getegid", lambda: 0)
...
def validate_user(val):
    ...
    if isinstance(val, int):
        ...
        if pwd is None:
            raise ConfigError("Named users are unavailable on this platform")
        try:
...
def validate_group(val):
    ...
    if isinstance(val, int):
        ...
        if grp is None:
            raise ConfigError("Named groups are unavailable on this platform")
        try:
...
class User(Setting):
    ...
    default = _geteuid()
    default_doc = "``os.geteuid()`` (or 0 when unavailable)"
    desc = """\
...
class Group(Setting):
    ...
    default = _getegid()
    default_doc = "``os.getegid()`` (or 0 when unavailable)"
    desc = """\

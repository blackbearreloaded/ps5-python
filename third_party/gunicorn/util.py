...
# PS5 does not currently expose the POSIX ``fcntl`` and ``pwd`` extension
# modules.  Keep Gunicorn's network/HTTP path importable and make the
# descriptor hardening best-effort; the PS5 launcher does not exec children.
try:
    import fcntl
except ImportError:
    fcntl = None
...
def set_non_blocking(fd):
    if fcntl is None:
        # Socket objects provide the portable implementation used by the
        # PS5 worker; integer descriptors are left unchanged.
        return

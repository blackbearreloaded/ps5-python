...
int main(int argc, char **argv)
{
    ...
    if (argc > 2)
        ...
        repl_port = port + 1;
    if (argc > 3)
        supervisor_port = strtoul(argv[3], NULL, 10);
    else if (port < 65533)
        supervisor_port = port + 2;
    if (port == 0 || port > 65535 || repl_port == 0 || repl_port > 65535 ||
        supervisor_port == 0 || supervisor_port > 65535 || repl_port == port ||
        supervisor_port == port || supervisor_port == repl_port)
        return 2;

    tcp_repl_port = (unsigned short)repl_port;
    if (app_manager_start((unsigned short)supervisor_port) != 0)
        return 1;
    ...
    daemon = MHD_start_daemon(MHD_USE_INTERNAL_POLLING_THREAD | MHD_USE_THREAD_PER_CONNECTION |
                              ...
                              (uint16_t)port, NULL, NULL, web_access_handler, NULL,
                              MHD_OPTION_NOTIFY_COMPLETED, web_request_completed, NULL,
                              MHD_OPTION_END);
    if (daemon == NULL)
    {
        app_manager_stop();
    ...
    printf("[launcher] app supervisor: 127.0.0.1:%lu\n", supervisor_port);
    fflush(stdout);
    while (!server_stop)
        sleep(1);

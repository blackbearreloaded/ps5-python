...
| Present in the current bundle inventory | 170 | The top-level module/package is in the current bundle tree or provided by the runtime's native/public path |
| Raw absent entries | 20 | The complete list below plus `site-packages` |
| Actual stdlib entries absent | 19 | Raw absent entries after excluding `site-packages` |
| Module-presence estimate | 170 / 189 = 89.9% (about 90%) | Presence only; it does not measure API parity or regression-test completeness |

The 19 actual absent entries are classified below. This is the complete
comparison set; the supported set is its complement and is covered by the
status sections that follow. `site-packages` is the 20th raw absence and is
...
`tomllib`, `trace`, `xmlrpc`, `zipapp`, `ntpath`, `nturl2path`,
`_threading_local`, `_pyio`, `_pylong`, `_py_abc`, `sre_compile`,
`sre_constants`, and `sre_parse`. It also includes `_strptime` and
...
`tests/stdlib/test_missing_stdlib.py`; the aggregate PS5 run passes this test.

| Added surface | Bundle status | Available checks | Remaining limit |
| --- | --- | --- | --- |
| `configparser`, `fileinput`, `modulefinder`, `netrc`, `pickletools`, `plistlib`, `pyclbr`, `sched`, `stringprep`, `tabnanny`, `trace`, `zipapp` | Official CPython 3.14.7 pure-Python wrappers | Import and bounded local API checks pass on PS5 | Full upstream regression coverage and unusual filesystem/CLI cases remain pending |
| `tomllib`, `xmlrpc` | Official CPython 3.14.7 packages | TOML parsing and XML-RPC marshal/response checks pass on PS5 without network or server execution | Full parser/protocol/server coverage remains pending |
| `_strptime`, `_markupbase` | Official CPython 3.14.7 private dependencies in the current bundle | Focused `time.strptime()` and `html.parser.HTMLParser` checks pass on PS5 | Full datetime/HTML parser regression coverage remains pending |
| `ntpath`, `nturl2path`, `_threading_local`, `_pyio`, `_pylong`, `_py_abc`, `sre_compile`, `sre_constants`, `sre_parse` | Official CPython 3.14.7 pure-Python helpers in the current bundle | Bounded path, URL, in-memory I/O, threading-local, integer, ABC, and regex checks pass on PS5 | Private/deprecated helpers retain upstream compatibility limits; full regression coverage remains pending |
...
The aggregate PS5 run completed with `CPYTHON_CORE_SUITE: PASS (68 scripts)`.
The `netrc` check accepts the platform's empty-string account field as the
equivalent of desktop CPython's `None` field for an omitted account.

The current 170-entry inventory leaves 19 absences: 17 blocked entries in the
packaging/bootstrap, GUI, PTY/terminal, platform-specific, and demo classes
above, plus the two deliberately unnecessary private/fallback files. The
current `_strptime` and `_markupbase` bundle entries cover the known public
dependency gaps; other private/fallback omissions do not automatically mean that
